  ...
  return (
    <form onSubmit={validateAndContinue} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {isPhysical && (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Referencias */}
            <div className="space-y-1 md:col-span-2">
              <label className="text-[10px] font-black uppercase" style={textColorStyle}>Referencias de entrega</label>
              <input
                type="text"
          ...
          {!isShippingAvailable && customerData.billing_address.postal_code.length >= 4 && (
            <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-xl flex gap-3 mt-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="space-y-2">
                <p className="text-sm font-bold text-amber-800" style={inputTextColorStyle}>
                  Zona fuera de cobertura automática
                </p>
                <p className="text-xs text-amber-700" style={inputTextColorStyle}>
      ...
      <div className="bg-gray-100 p-4 rounded-2xl flex gap-3 border border-gray-200">
        ...
        <div className="space-y-1">
          <h4 className="text-xs font-black uppercase" style={textColorStyle}>Nota Importante sobre Facturación</h4>
      ...
      {errors.length > 0 && (
        <div className="bg-red-50 p-4 rounded-xl border border-red-100 animate-pulse">
          <div className="flex items-center gap-2 mb-2 font-bold text-sm">
            ...
            <span className="text-red-600" style={inputTextColorStyle}>Faltan datos por completar:</span>
          </div>
          <ul className="list-disc list-inside text-xs space-y-1 font-medium">
            {errors.map((err, i) => <li key={i} className="text-red-500" style={inputTextColorStyle}>{err}</li>)}
          </ul>
        </div>
      )}
      ...
      <button
        ...
        className={`w-full py-4 rounded-2xl font-black flex items-center justify-center gap-3 transition shadow-xl ${
          (isPhysical && !isShippingAvailable)
            ...
            : 'bg-black hover:bg-gray-800 hover:scale-[1.01] active:scale-95 shadow-gray-200 text-white'
        }`}
      >
        Continuar al Pago
        <ArrowRight size={20} />
